# coding=utf-8
"""Implementation of MI-DI-FGSM attack."""
...
tf.flags.DEFINE_string(
    "arch", "", "arch: inc_v3, inc_res_v2, res_101, res_152."
)
...
tf.flags.DEFINE_string(
    "dhf_layers", "-1", "the layers will be applied to DHF"
)
...
def graph(x, x_mixup, y, i, x_max, x_min, grad, if_mixup):
    ...
    with slim.arg_scope(model_arg_scope()):
        ...
        if FLAGS.method == "dhf":
            ...
            mixup_features = dict()
            x_mixup_, rnds = input_diversity(x_mixup)
            ...
            # use the same input diversification as the x_mixup to make sure the alignment of features
            x_, _ = input_diversity(x, rnds=rnds)
...
def input_diversity(x, rnds=None):
    if rnds is not None:
        rnd = rnds[0]
    else:
        rnd = tf.random_uniform((), FLAGS.image_width, FLAGS.image_resize, dtype=tf.int32)

    rescaled = tf.image.resize_images(x, [rnd, rnd], method=tf.image.ResizeMethod.NEAREST_NEIGHBOR)
    ...
    padded.set_shape((x.shape[0], FLAGS.image_resize, FLAGS.image_resize, 3))
    ret = tf.cond(tf.random_uniform(shape=[1])[0] < tf.constant(FLAGS.prob), lambda: padded, lambda: x)
    ret = tf.image.resize_images(ret, [FLAGS.image_height, FLAGS.image_width],
                                 method=tf.image.ResizeMethod.NEAREST_NEIGHBOR)
    return ret, (rnd, pad_top, pad_left)
